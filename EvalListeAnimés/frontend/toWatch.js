        $(document).ready(() => {
            ...
            function getAnimes() {
                ...
                $.ajax({
                    ...
                    success: (result) => {
                        let html = '';
                        result.forEach(obj => {
                            html += '<p>id: ' + obj.id + ' Nom: ' + obj.name + ' Année de commencement: ' + obj.year + '</p>';
                        })
            ...
            function getAnimeById() {
                ...
                $.ajax({
                    type: "GET",
                    url: apiBaseUrl + 'toWatch/'+ id,
                    contentType: "application/json; charset=UTF-8",
                    dataType: 'json',
                    success: (result) => {
                        let html = '';
                            html += '<p>id: ' + result.id + ' Nom: ' + result.name + ' Année de commmencement: ' + result.year + '</p>';
                        $("#one-anime").html(html)
                    },
                    error: (xhr, status, error) => {
                        console.log(xhr);
