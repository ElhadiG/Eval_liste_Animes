...
exports.getAll=(request,response) => {
    fs.readFile(path, (err,data) => {
        if (err){
            ...
            response.status(200).json(JSON.parse(data).toWatch);
        };
    });
};
...
exports.createData= (request,response) => {
    fs.readFile(path, (err, data) => {
        if(err){
            ...
            //enregistre le tableau toWatch seulement
            const toWatchanimes = allParsedData.toWatch
            //crée un tableau d'id grace a map
            const toWatchAnimeIds= toWatchanimes.map((obj)=> obj.id)
            //math.max prend l'id le plus grand 
            const maxId= Math.max(...toWatchAnimeIds)
            //je rajoute +1 a la valeur de l'id stocké
            const newId= maxId+1
            // const tri=
            //rajouter cette donnée a partir de ce qu'on a ecrit dans le body
            allParsedData.toWatch.push({
                "id":newId,
...
exports.deleteData=(request, response)=>{
    ...
    fs.readFile(path,(err,data)=>{
        ...
        //stocker les données existantes
            ...
            const animeToDelete= allParsedData.toWatch.find(
        ...
        if(!animeToDelete){
            ...
            allParsedData.toWatch=allParsedData.toWatch.filter(
                (obj)=> obj.id != parseInt(request.params.id));
...
exports.getById=(request,response) =>{
    ...
    fs.readFile(path ,(err,data)=>{
        ...
        if (err){
            ...
            const anime= allParsedData.toWatch.find(
                (obj) => obj.id === parseInt(request.params.id)
            )
            //si on trouve cet id
            if (anime){
